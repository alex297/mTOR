...
for (i in 1:length(cells)) {
  ...
  write.csv(as.matrix(output), file = paste0(c("ins_ref_",cell,".csv"), collapse = ''), row.names=F)
  #PTEN 50%
  output <- data.frame(matrix(ncol=length(insul),nrow = 27))
  colnames(output) <- paste0("ins",signif(insul,2))
  PTEN <- ini[3,i+1]/2
  ...
  output <- data.frame(variable=colnames(out0)[-1],output)
  write.csv(as.matrix(output), file = paste0(c("ins_PTEN05_",cell,".csv"), collapse = ''), row.names=F)
  #PTEN 200%
  output <- data.frame(matrix(ncol=length(insul),nrow = 27))
  colnames(output) <- paste0("ins",signif(insul,2))
  PTEN <- ini[3,i+1]*2
  ...
  output <- data.frame(variable=colnames(out0)[-1],output)
  write.csv(as.matrix(output), file = paste0(c("ins_PTEN2_",cell,".csv"), collapse = ''), row.names=F)
  PTEN <- ini[3,i+1]
  #TSC 50%
  output <- data.frame(matrix(ncol=length(insul),nrow = 27))
  colnames(output) <- paste0("ins",signif(insul,2))
  ...
  output <- data.frame(variable=colnames(out0)[-1],output)
  write.csv(as.matrix(output), file = paste0(c("ins_TSC05_",cell,".csv"), collapse = ''), row.names=F)
  #TSCK 200%
  output <- data.frame(matrix(ncol=length(insul),nrow = 27))
  colnames(output) <- paste0("ins",signif(insul,2))
  TSC <- ini[5,i+1]*2
  ...
  output <- data.frame(variable=colnames(out0)[-1],output)
  write.csv(as.matrix(output), file = paste0(c("ins_TSC2_",cell,".csv"), collapse = ''), row.names=F)
}
