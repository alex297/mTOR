...
ini <- data.frame(matrix(ncol=1001,nrow = 10))
colnames(ini) <- c("gene",paste0("c",seq(1,1000,1)))
ini$gene  <- c("IR","PTEN","IRS1","PKB","TSC","mTORC1","mTORC2","S6K","PDGFR","NRGR") 
for (j in 1:10) {
  for (i in 1:1000) {
    ini[j,i+1] <- draws[(j-1)*1000+i]
  }
}
rownames(ini) <- NULL
#write.csv(as.matrix(ini), file = "mod_abundances_random_1221.csv", row.names=F)
...
S6Ka <- data.frame(matrix(ncol=1000,nrow = 3))
colnames(S6Ka) <- colnames(ini)[-1]
...
for (i in 1:1000){
  ...
  TSC <- ini[5,i+1]
  PTEN <-ini[2,i+1]
  xstart <- c(IRm=ini[1,i+1],IRins=0,IRmYP=0,IRiYP=0,IRi=0,IRS1=ini[3,i+1],IRS1YP=0,IRS1YPS307=0,IRS1S307=0,X=1,Xp=0,PKB=ini[4,i+1],PKBT308=0,PKBS473=0,PKBT308S473=0,
              TORC1=1,TORC1a=0,TORC2=1,TORC2a=0,S6K=ini[8,i+1],S6KT389=0,S6=1,S6S235=0,r=1,c1=0,c2=0,L=0)
  ...
  parms <- c(insulin=ins,k1a=0.633,k1c=0.877,k1d=31,k1f=184000,k1g=1940,k1r=0.547,k2_0=0.0423,k2a=323,k2b=3420,
             k2c=576000,k2d=281,k2f=2.91,k2g=0.267,k3_0=0.01,k3a=0.0001,k3b=0.0988,k4a=579000,k4b=34.8,k4c=446,k4f=30,k4h=0.536,k5b=24.8,
             k5c=2,k5d=1.06,k9b1=0.0444,k9b2=31,k9f1=0.9,k9f2=333,km9=58.7,k5a1=0.03,kiTSC=0.001,k5=40,
             kmx=0.07,kx=30,KDL=1.5,kf=1,ke=0.2,kLmax=0.011,KML=1.66,kPI3K=0.3,aPI3K=8000,k4d=30,k5e=0.2,PTEN=1,kPTEN=1e-5,
             k4_0=4,k4c_0=0.3)
...
for (i in 1:1000){
  ...
  ode(
    ...
    as.data.frame() -> out
  S6Ka[3,i] <- max(out[721,22])
}  

S6Ka <- data.frame(ligand=c("insulin","pdgf","nrg"),S6Ka)
#write.csv(as.matrix(S6Ka), file = "S6Ka_refmod_random_1221.csv", row.names=F)
